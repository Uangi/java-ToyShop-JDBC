...
public class ProductDAO {		// DB에 연결 처리 클래스 (Data Access Object)
	...
	// 제품 추가 메서드 수정
	 ...
	 public int purchase(ProductDTO dto) {
		 		int result = 0;
...
//        	pstmt.executeUpdate()
    ...
    // 재고 차감 메서드
    public void subtractFromStock(ProductDTO dto) {
        Connection conn = DBConn.getConnection();
        PreparedStatement pstmtSubtractFromStock = null;

        try {
            // 재고에서 수량 차감
            String subtractFromStockSQL = "UPDATE Product SET quantity = quantity - ? WHERE product_name = ?";
            pstmtSubtractFromStock = conn.prepareStatement(subtractFromStockSQL);
            pstmtSubtractFromStock.setInt(1, dto.getQuantity()); // 구매 시 입력한 수량
            pstmtSubtractFromStock.setString(2, dto.getProductName());
            pstmtSubtractFromStock.executeUpdate();

            System.out.println("재고에서 상품 수량 차감 완료.");

        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
            // close resources
            DBConn.close(conn, pstmtSubtractFromStock);
        }
    }
